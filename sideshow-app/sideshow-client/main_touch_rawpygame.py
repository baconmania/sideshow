...
class Sideshow():
  ...
  def render_cpu_gpu_temps_page(self):
    ...
    text_surface = self.font_big.render('%s°' % self.metrics['temps']['cpu'], True, Sideshow.WHITE)
    ...
    text_surface = self.font_big.render('%s°' % self.metrics['temps']['gpu'], True, Sideshow.WHITE)
